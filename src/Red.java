...
public class Red extends Simulation {
    /***************************************************************************
    *                                - DIMENTIONS -                            *
    ***************************************************************************/
    ...
    public void reset(){
        ...
        for (int x = 0; x < xdim; x++){
            for (int y = 0; y < ydim; y++){
                int r_max = Math.min(xdim, ydim) /  20;
                int p1x = xdim/2 - 4 * r_max - x;
                int p1y = ydim/2 - 4 * r_max - y;
                double r1 =  Math.sqrt(p1x*p1x + p1y*p1y);
                int p2x = xdim/2 + 4 * r_max - x;
                int p2y = ydim/2 - 4 * r_max - y;
                double r2 =  Math.sqrt(p2x*p2x + p2y*p2y);
                int p3x = xdim/2 - 4 * r_max - x;
                int p3y = ydim/2 + 4 * r_max - y;
                double r3 =  Math.sqrt(p3x*p3x + p3y*p3y);
                int p4x = xdim/2 + 4 * r_max - x;
                int p4y = ydim/2 + 4 * r_max - y;
                double r4 =  Math.sqrt(p4x*p4x + p4y*p4y);

                barrier[x][y] = (r1 < r_max || r2 < r_max || r3 < r_max || r4 < r_max);
    ...
    public void advance(){
        for (int x = 0; x < xdim; x++){
            for (int y = 0; y < ydim; y++){
                ...
                double r_max = Math.min(xdim, ydim) * 0.10;
                double r_scale = r / r_max;
                if (r < r_max){
                    double amount = 0.005 * r_scale;
    ...
    ***************************************************************************/
    ...
    public void draw(){
        for (int x = 0; x < xdim; x++){
            for (int y = 0; y < ydim; y++){
                float b = Math.min(20.0f * (float) speed2[x][y], 1.0f);
                Color color = Color.getHSBColor(0.0f,1.0f,b);
                double r = 0.5;
